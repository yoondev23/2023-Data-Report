...
# 부산지역
geo_poly = folium.GeoJson(data=sd_busan, style_function=lambda x: {'line_weight' : 0.004,'fillColor': 'red', 'fillOpacity': 0.1})
geo_poly.add_to(m)
...
for polygon in intersect_poly_lst_move:
    ...
    geo_poly = folium.GeoJson(data=polygon_json, style_function=lambda x: {'fillColor': '#FFBF00', 'fillOpacity': 0.4,'lineColor': '#3A2F0B'})
    geo_poly.add_to(m)
